...
/*   Updated: 2024/09/22 00:20:17 by ide-dieg         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_buffer_lst	*ft_addnewlst(t_buffer_lst *lst)
{
	...
	if (node->content == 0)
	{
		ft_lstclear_g(&lst, free);
		ft_lstclear_g(&node, free);
		return (0);
	}
...
char	*get_next_line_2(int fd, t_buffer_lst **buffer)
{
	...
	while (ft_strnchr(ft_lstlast_g(*buffer)->content, '\n',
			ft_lstlast_g(*buffer)->lencontent) == -1)
	{
		*buffer = ft_addnewlst(*buffer);
		temp = ft_lstlast_g(*buffer);
		temp->lencontent = read(fd, temp->content, BUFFER_SIZE);
		if (ft_lstlast_g(*buffer)->lencontent <= 0)
		{
			line = lstjoin(*buffer, ft_lstlast_g(*buffer)->lencontent - 1);
			ft_lstclear_g(buffer, free);
			return (line);
		}
	}
	endline = ft_strnchr(ft_lstlast_g(
				*buffer)->content, '\n', ft_lstlast_g(*buffer)->lencontent);
...
char	*get_next_line(int fd)
{
	...
	line = get_next_line_2(fd, &buffer);
	if (line == 0)
		ft_lstclear_g(&buffer, free);
	return (line);
}
...
int main()
{
	...
	while (cont < 9)
	{
		read(fd, ft_lstlast_g(lst)->content, BUFFER_SIZE);
		printf("%d\n", ft_strnchr
			(ft_lstlast_g(lst)->content, '\n', BUFFER_SIZE));
		ft_addnewlst(lst);
		cont++;
	}
	read(fd, ft_lstlast_g(lst)->content, BUFFER_SIZE);
	...
	printf("%d\n", ft_lstsize(lst));
	char *line = lstjoin(lst, ft_strnchr(ft_lstlast_g(lst)->content, '\n',
	 BUFFER_SIZE));
	printf("%s", line);
	lst = cleanbuffer(lst, ft_strnchr(ft_lstlast_g(lst)->content, '\n',
	 BUFFER_SIZE));
	printf("%d\n", lst->lencontent);
	cont = 0;
...
t_list	*ft_lstlast_g(t_list *lst)
{
	if (lst == 0)
		return (0);
	while (lst -> next != 0)
...
char	*get_next_line(int fd)
{
	...
	{
		buffer = ft_addnewlst(buffer);
		ft_lstlast_g(buffer)->lencontent = read(fd,
		 ft_lstlast_g(buffer)->content, BUFFER_SIZE);
		if (ft_lstlast_g(buffer)->lencontent <= 0)
		{
			line = lstjoin(buffer, ft_lstlast_g(buffer)->lencontent - 1);
			ft_lstclear_g(&buffer, free);
	...
	endline = ft_strnchr(ft_lstlast_g(buffer)->content, '\n',
	 ft_lstlast_g(buffer)->lencontent);
	line = lstjoin(buffer, endline);
	if (line == 0)
		return (0);
